...
Future<void> getItemShop() async {
  ...
  if((response.statusCode>=200&& response.statusCode<300)){
    ...
    log(listShop.length.toString());log("*****************************************************************");
...
Future<void> getItemGas() async {
  ...
  var url = Uri.parse(
      "https://news.wasiljo.com/public/api/v1/delivery-boy/categories/2/subcategories");
...
void initState(){
  appUserData.data!.deliveryBoy!.categoryId==1?  getItemShop():
getItemGas();
}
...
String errorText='';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: Text(  Translator.translate('Add New Item'),style: TextStyle(color:primaryColor,fontWeight: FontWeight.bold,)),
      ...
      body: SingleChildScrollView(
        child: Form(
          ...
          child: Padding(
            ...
            child: Column(
              children: [
                ...
                appUserData.data!.deliveryBoy!.categoryId==1?      Container(
          height:
          ...
          child: DropdownButton<SubCategory>(isExpanded: true,
          underline: Container(),
          value: selectedShop,
          ...
          padding: EdgeInsets.all(10),
          child: DropdownButton<SubcategoriesGas>(isExpanded: true,
          underline: Container(),
          value: selectedGas,
          borderRadius: BorderRadius.circular(20),
          dropdownColor: Colors.white,
          ...
          items: subcategoryGas
              .map((shop) =>
          DropdownMenuItem<SubcategoriesGas>(
          value: shop,
          child: Text(shop!.title.ar),
          ))
              .toList(),
                ...
                CustomTextField(
                  validator: (value) {
                    if (value == null || value.isEmpty) {
                      return Translator.translate('Please enter your total amount');
                    }
                  },
                ...
                SizedBox(height: 12,),
                CommonViews().createButton(title:  Translator.translate('Add New Item'), onPressed: () async {
                  SharedPreferences prefs=await SharedPreferences.getInstance();
                  var token=prefs.getString('token');
                  var headers = {
                  ...
                  var response =await http.post( Uri.parse('https://news.wasiljo.com/public/api/v1/delivery-boy/subcategories/select')

                      ,body : json.encode([
                      {
                      "sub_category_id": appUserData.data!.deliveryBoy!.categoryId==1?selectedShop!.id.toString():selectedGas!.id,
