...
//   /*-----------------   Log In     ----------------------*/
//
//    Future<MyResponse> loginUser(String phone, String password) async {
//     //Get FCM
...
//       required String nameEnglish,      required String nameArabic,  required String agencyEnglish,      required String agencyArabic,  required String email,
//       required String password, mobile, category,shop,File? drivingLicense,File? profilePic , carNum}) async {
//     //Get FCM
//     PushNotificationsManager pushNotificationsManager =
//         PushNotificationsManager();
//     await pushNotificationsManager.init();
...
//
//     String registerUrl = 'https://news.wasiljo.com/public/api/v1/delivery-boy/register';
//
//     final dio = Dio();
//
//
...
//       MapEntry('agency_name[en]', agencyEnglish==null?"":agencyEnglish),
//       MapEntry('agency_name[ar]', agencyArabic==null?"":agencyArabic),
//
//       MapEntry('name[ar]', nameArabic),
//       MapEntry('name[en]', agencyEnglish),
...
//             "https://news.wasiljo.com/public/api/v1/delivery-boy/register",
//             data: formData ,
...
//   /*-----------------   Save user in cache   ----------------------*/
//
//    saveUser(Map<String, dynamic> user,String? type) async {
//     await saveUserFromDeliveryBoy(DeliveryBoy.fromJson(user,type.toString()));
//   }
//
//    saveUserFromDeliveryBoy(DeliveryBoy deliveryBoy) async {
//     SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
...
//   /*-----------------   Get user from cache     ----------------------*/
//
//    Future<DeliveryBoy> getUser() async {
//     SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
//
...
//           TextSpan(
//               text:
//                   "After testing please logout, because thfere is many user testing with same IDs so it can be possible that you can get unnecessary notifications",
...
import '../../utils/ui/prgress_hud.dart';
...
class AuthControllerr extends GetxController {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        SharedPreferences sharedPreferences =
            await SharedPreferences.getInstance();
        Map<String, dynamic> data = json.decode(response.body!);
        boysDeliveryData = DeliveryBoyModel2.fromJson(data);
        print(
            "================${boysDeliveryData?.data?.deliveryBoy?.categoryId}xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");
        //save to firebase
        await saveDeliveryBoyData(boysDeliveryData!);
        ...
        // await saveUser(user);
        await sharedPreferences.setString('token', token);
        await sharedPreferences.setString('tokensave', token);
  ...
  Future<void> saveDeliveryBoyData(DeliveryBoyModel2 data) async {
    try {
